...
Deterministic fuzzy typo tolerance is a bounded fallback, not semantic search. If the original AND search returns no filtered results, unknown terms of at least four characters may be corrected to terms in that index's wiki-derived vocabulary, and corrected AND is tried before weakening the original query to OR. Corrected OR is only tried if original OR is also empty. Known terms and shorter tokens are never changed. Maximum Levenshtein distance is `1` for lengths 4–5 and `2` for lengths 6 and above. Candidates are selected by distance, field priority, document frequency, then alphabetical order. Use `--fuzzy` to search the corrected query directly with AND then OR, or `--no-fuzzy` to use only original AND then OR. These options are mutually exclusive.
